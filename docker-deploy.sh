...
if [ "$1" == "development" ]; then
	...
	docker run -v /etc/letsencrypt:/etc/letsencrypt -v $CACHE_PATH:/app/RemoteCache.Worker/Cache --name "remote-cache" -d -p 8011:8081 -p 8080:8082 --restart on-failure "remote-cache"
fi
